 ...
 * @Last Modified time: 2020-08-17 04:25:54
 */
 ...
 */
const rcodeConfig = rcodeConfigGenerator();


/**
 * 返回 R 相关配置信息
 */
function rcodeConfigGenerator() {
    if ( !PRODUCTION )
    return {
        // 注意转义 \\\\ => \\ 注意末尾的反斜杠
    ...
    return {
        dataPath: '/home/www/htdocs/ena/xlsx/',
        dataName: 'encode.csv',
        outputName: 'output.html',
        rPath: '../R/', // 应当是相对于 saver.php的，注意末尾的斜杠
        rName: 'ena_auto.R'
...
const prepareRCodes = (function() {
    ...
    function baseCodes() {
        ...
        base += `STEM.data = read.csv("${rcodeConfig.dataPath}${rcodeConfig.dataName}", 1)\n\n`;
    ...
    function parameterCodes() {
        ...
        if (parameter.conversation.length) {
            codes += note('确定会话');
            codes += `conversation = as.data.frame(STEM.data[,c(${decorateValues(parameter.conversation).join(',')})])\n\n`
        }
    ...
    function saveCodes(codes) {
        ...
        $.get('./php/saver.php', {codes: codes, path: path}, res => {
            if (res) {
                console.log('saveCodes - successfully');
                // 源代码区域显示代码
                $('#sourcecode').html(codes);
                // generate 中 listen 需保持监听
